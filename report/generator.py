...
async def generate_wechat_report(
    ...
    wechat_prompt = (
        ...
        f"## 微信版定位（最重要）\n\n"
        f"这是**朋友转述版**——你懂命理，看完对方的盘之后，"
        f"用朋友的口吻在微信上告诉对方最重要的事。\n"
        f"不是正式报告，不是产品，不是客服话术。\n"
        f"是一个懂你的人，用最直接的方式提醒你。\n\n"
        f"## 结构（自然段落，不要标题/编号）\n\n"
        f"1. **开头**：我看完你的盘，最想提醒你一句话是……（一句话定命，有记忆点）\n"
        f"2. **核心画像**：你不是不行，而是哪里容易卡住（用比喻讲清底层模式）\n"
        f"3. **当前阶段**：最近两三年怎么走（该蓄力还是该冲，主节奏是什么）\n"
        f"4. **重点提醒**：事业/钱/感情/身体各一两句狠话（直接、具体、有用）\n"
        f"5. **结尾**：朋友式鼓励（温暖收尾，给信心，不是客套）\n\n"
        f"## 风格铁律\n\n"
        f"1. 语气自然、温暖、直接，像跟好朋友说话\n"
        f"2. 命理术语点到为止，立刻翻译成人话\n"
        f"3. 比喻精准有力（如'你不是能力不够，是电量不够'）\n"
        f"4. 建议具体但不堆砌（每个领域1-2句最狠的就够）\n"
        f"5. 所有事实必须与命理师版一致，不能无中生有\n\n"
        f"## 绝对禁止出现的内容\n\n"
        f"- 表格\n"
        f"- markdown 标题（#）\n"
        f"- P0/P1/P2 等优先级标签\n"
